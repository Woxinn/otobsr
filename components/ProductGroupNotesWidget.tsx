...
import { StickyNote, Plus, Trash2, Pencil, Check, X, Calendar, AlertCircle, Loader2 } from "lucide-react";
import { createProductGroupNote, updateProductGroupNote, deleteProductGroupNote } from "@/app/actions/product-group-notes";
...
interface ProductGroupNotesWidgetProps {
  groupId: string;
  notes: Note[];
  canEdit: boolean;
}

export default function ProductGroupNotesWidget({
  groupId,
  notes,
  canEdit,
}: ProductGroupNotesWidgetProps) {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      await createProductGroupNote(groupId, trimmed);
      setNewNote("");
    } catch (err: any) {
  ...
  const handleSaveEdit = async (noteId: string) => {
    ...
    setIsSaving(true);
    try {
      await updateProductGroupNote(noteId, groupId, trimmed);
      setEditingId(null);
      setEditingText("");
    } catch (err: any) {
  ...
  const handleDelete = async (noteId: string) => {
    ...
    setIsDeleting(true);
    try {
      await deleteProductGroupNote(noteId, groupId);
      setDeletingId(null);
    } catch (err: any) {
        ...
        <form onSubmit={handleAdd} className="mt-4">
          <div className="relative rounded-xl border border-slate-200 bg-slate-50/50 p-1.5 focus-within:border-teal-500 focus-within:bg-white focus-within:ring-1 focus-within:ring-teal-500 transition-all duration-200">
            <textarea
              ...
              placeholder="Kategori için hızlıca bir not yazın... (Enter ile kaydet, Shift+Enter ile alt satır)"
              onKeyDown={(e) => {
