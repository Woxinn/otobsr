...
export function ArchiveButton({ orderId, archived }: Props) {
  ...
  const handleClick = () => {
    const msg = archived
      ...
      : "Arşivlemek istiyor musunuz?";
    if (!confirm(msg)) return;
    start(async () => {
      await fetch("/api/orders/archive", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: orderId, archived: !archived }),
      });
      router.refresh();
    });
  };
  ...
  return (
    ...
    >
      {pending ? "Bekleyin..." : archived ? "Arşivden çıkar" : "Arşivle"}
    </button>
  );
}
